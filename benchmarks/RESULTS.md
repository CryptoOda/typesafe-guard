...
directly rather than computed from a separate pricing table. Jev's cost is
computed from TypeSafe's published pricing (input $0.042/1M tokens, output
free) applied to its measured token counts above — not from an API-reported
`cost` field, since Jev's SDK response doesn't include one.

| model                          | requests | input tok/req | output tok/req | total cost | cost/1k requests | mean latency | median latency | p95 latency |
|----------------------------------|----------|----------------|------------------|------------|--------------------|----------------|------------------|--------------|
| Jev (`jev-latest`, via TypeSafe)  | 50       | 473            | 82               | $0.000994  | **$0.0199**        | 0.358s         | 0.326s           | 0.460s       |
| `mistralai/mistral-small-2603`    | 50       | 330            | 43               | $0.002908  | $0.0582 (2.9x Jev) | 3.58s          | 3.47s            | 4.88s        |
| `openai/gpt-4o-mini`              | 50       | 319            | 42               | $0.003650  | $0.0730 (3.7x Jev) | 1.99s          | 2.02s            | 3.11s        |
| `anthropic/claude-haiku-4.5`      | 50       | 341            | 49               | $0.029301  | $0.5860 (29.5x Jev)| 1.48s          | 1.38s            | 1.81s        |

Observations:

- **Jev is the cheapest of the four, and by a wide margin over one of them.**
  At $0.0199/1k requests it's roughly 3-4x cheaper than gpt-4o-mini or
  mistral-small, and ~30x cheaper than claude-haiku-4.5, despite using more
  tokens per request (see below) — Jev's free output pricing and low input
  rate more than make up for the higher token count.
- **Cost also varies ~10x by provider among the three general LLMs alone.**
  `claude-haiku-4.5` costs about 8x `gpt-4o-mini` and 10x `mistral-small`
  per request for the same task — provider choice matters as much as the
  Jev-vs-general-LLM choice does.
...
- **Jev uses more tokens per request than these general LLMs did** (473 in /
  ...
  definitions themselves (same instructions text was used in both), and it's
  cheaper anyway once actual per-token pricing is applied, which is exactly
  why this section reports $ rather than stopping at token counts.
